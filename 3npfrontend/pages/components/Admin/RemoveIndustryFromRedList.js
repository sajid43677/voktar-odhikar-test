...
export default function RemoveIndustryFromRedList(props) {
  console.log(props.Industry);
  const router = useRouter();
  const form = useForm({
    defaultValues: {
        name: props.Industry.name,
      reason: props.Industry.reason,
      issuer: props.Industry.issuer,
      redlistedserialnumber:props.Industry.redlistedserialnumber,
    },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.delete(
        ...
        `admin/redlistindustry?serialnumber=${props.Industry.redlistedserialnumber}`,
        {
      ...
      <div className="">
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              {updated && (
                <a className="text-sm  text-green-600">Industry Removed From RedList</a>
              )}
            </div>
          </form>
        </div>
